...
public class VOX30ONT implements IProfile
{
    private static String convertEnvironmentName(String appName)
    {
        if(XStringUtil.isBlank(appName) || true == appName.contains("webgui"))
        {
            return "WebGUI";
        }
    ...
    throws Throwable
    {
        ...
        Map<?,?> dataModel1 = UbusManager.getInstance().fire(
            ...
            mac, 
            "get", 
            UbusUtil.buildPath("Services", "Management", "LCM", "ExecutionEnvironments", "WebGUI", "Packages"),
            XStringUtil.BLANK, 
            10 * 1000L);
        dataModels.add(dataModel1);

        Map<?,?> dataModel2 = UbusManager.getInstance().fire(
            serial, 
            mac, 
            "get", 
            UbusUtil.buildPath("Services", "Management", "LCM", "ExecutionEnvironments", "Security", "Packages"),
            XStringUtil.BLANK, 
            10 * 1000L);
